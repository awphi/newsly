...
stories.updateImage = function (id, img) {
    ...
    fs.access(file, fs.constants.F_OK, (err) => {
        if (err) {
            console.log(`Error encountered when reading image of story ${id}, ${img}: ${err}`);
            return;
        }
...
stories.updateComments = function (id) {
    stories.getJson(path.join(__dirname, 'data', id, 'comments.json'))
        .then((json) => {
            stories.comments[id] = json;
            return json;
        })
        .catch((err) => console.log(`Error encountered when reading comments of story ${id}: ${err}`));
};
...
stories.update = function () {
    fs.readdir(path.join(__dirname, 'data'), function (err, files) {
        ...
        files.forEach(i => {
            ...
            stories.getJson(path.join(__dirname, 'data', i, 'post.json'))
                .then((json) => {
                    stories.cache[i] = json;
                    return json;
                })
                .then(() => stories.updateImages(i))
                .catch((err) => console.log(`Error encountered when reading story: ${err}`));
